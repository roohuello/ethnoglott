---
name: grill-me
description: "Adversarial plan interview before implementation. Use for grill me, stress test, poke holes, challenge my plan."
---
...
Combines phase discipline with the frontier-rounds mechanic from
...
- Before breaking the plan into delegated sub-tasks

Do NOT use for existing code (use a code-review workflow instead) or simple
one-off tasks.
...
it yourself with file search / file read / terminal commands — or dispatch a
subagent for a heavy exploration. Never ask the user for
...
1. **Asking questions out of dependency order.** A question that depends on
   ...
   later round.
2. **Skipping the codebase.** Find facts in code with available tools instead
   of asking the user.
